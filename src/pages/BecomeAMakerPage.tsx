...
export const BecomeAMakerPage: React.FC = () => {
  const navigate = useNavigate();
  const { currentUser, userRole, isApproved, showToast } = useApp();
    ...
    async function loadExistingApp() {
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          ...
          setExistingApplication(pending || data[0]);
        }
      } catch (err: any) {
        console.warn('Could not query maker_applications:', err.message);
      } finally {
        setLoadingApp(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!currentUser) {
      navigate('/login?redirectTo=/become-a-maker');
      return;
    }
    ...
    setSubmitting(true);
    try {
      // Check for an existing pending application first
      ...
      if (!checkErr && check && check.length > 0) {
        setErrorMsg('You already have a pending application under review.');
      ...
      // Submit new application to Supabase
      const { data, error } = await supabase
        .from('maker_applications')
        .insert({
      ...
      if (error) {
        // Fallback for local demo simulation if Supabase credentials are in test mode
        setExistingApplication({
          status: 'pending',
          school,
          grade_class: gradeClass,
        ...
        setSuccessMsg('Your application has been logged and submitted for Master Admin review!');
        showToast('Maker application submitted!', 'sparkles');
      } else {
        setExistingApplication(data);
        setSuccessMsg('Application successfully submitted! Master Admin will review your craft portfolio.');
        showToast('Application sent to Master Admin!', 'sparkles');
      }
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-600 max-w-xl mx-auto leading-relaxed">
            Showcase your handcrafted terracotta, textiles, woodwork, and art. Receive fair compensation directly with zero listing fees.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-[#e7e0d8] shadow-xs space-y-2">
            ...
            <p className="text-[11px] text-slate-500 leading-relaxed">
              Every maker is verified by our administrative team to ensure quality and authenticity.
            </p>
          </div>
        </div>
        ...
        {!currentUser ? (
          ...
          <div className="bg-white rounded-3xl p-8 sm:p-10 border border-[#e7e0d8] shadow-sm text-center space-y-5">
            ...
            <Link
              to="/login?redirectTo=/become-a-maker"
          ...
          <div className="bg-white rounded-3xl p-8 sm:p-10 border border-amber-200 shadow-sm space-y-5">
            ...
            <div className="p-3.5 rounded-2xl bg-blue-50/70 border border-blue-200 text-xs text-blue-900 flex items-center gap-2.5">
              ...
              <span>
                You do not need to submit again. Once approved by Master Admin (ssumollah@gmail.com), you will automatically gain access to <strong>/maker</strong>.
              </span>
            </div>
          </div>
        ) : (
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-10 border border-[#e7e0d8] shadow-sm space-y-6">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">
                Applying as <strong className="text-slate-700">{currentUser.email}</strong>. Please provide accurate details.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>{submitting ? 'Submitting Application...' : 'Submit Maker Application'}</span>
                </button>
              </div>
            </form>
          </div>
